...
# def random_u_lie_algebra(N, rng=None):
#     """
#     Sample a random element of the Lie algebra u(N),
#     i.e. an anti-Hermitian N x N matrix.
#     """
#     if rng is None:
#         rng = np.random.default_rng(0)

#     # GUE Hermitian matrix
#     X = rng.standard_normal((N, N)) + 1j * rng.standard_normal((N, N))
#     H = (X + X.conj().T) / 2

#     # Anti-Hermitian element
#     A = 1j * H
#     return A/np.sqrt(N)


# def _one_rde_simulation(rng: np.random.Generator, path_increments: np.ndarray, N: int) -> complex:
#     d =  path_increments.shape[0]
#     # sample A_1, ..., A_d in Lie(U(N))
#     A = [random_u_lie_algebra(N, rng) for _ in range(d)]
